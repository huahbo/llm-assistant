...
**新 Agent 启动时必须按顺序读取以下文件，然后执行 §16.3 检查清单验证基线：**
...
## 18) 当前开发状态索引（Agent 交接必读）

> **本节不再存放详细状态。** 详细状态已迁移到独立文件，本节仅作目录指引。
> 每轮结束由主控 Agent **更新 `docs/dev-status.md`**，不修改本节。

### 文档分层结构

| 文件 | 层级 | 内容 | 读取频率 |
|------|------|------|----------|
| `docs/dev-status.md` | **活跃层** | 当前基线、活跃 TODO、最新 5 条提交、架构约束 | **每次必读** |
| `docs/交接状态卡.md` | **活跃层** | 当前接力轮次、已完成项、待验证步骤 | 接手时读 |
| `docs/实施过程记录.md` | **进度层** | 每轮详细实施记录（最新在最前，保留最近 4-5 条） | 读最新 3 条 |
| `docs/实施过程记录-归档-2026-04.md` | **进度层（归档）** | 2026-04-20 ~ 2026-04-27 全量记录 | 按需回溯 |
| `docs/completed-log.md` | **存档层** | 已完成功能完整列表（100+ 条） | 按需查阅 |
| `docs/archive/` | **存档层** | 历史调研与差距分析文档（已归档） | 一般不读 |
