...
import AgentToolsPane from "./modules/agent/AgentToolsPane";
        ...
        <div className={`module-viewport${activeModule === "ask" ? " module-viewport--ask" : ""}${activeModule === "agent" ? " module-viewport--agent" : ""}${activeModule === "agent" && agentDebugPanelOpen ? " module-viewport--agent-debug" : ""}`}>
          ...
          {activeModule === "inbox" && (
            <InboxModule
              ...
              ingesting={ingesting}
              supportedModesText={overview ? overview.supported_modes.map(formatBackendMode).join(" / ") : "浏览器预览"}
              currentModeLabel={overview ? formatBackendMode(overview.mode) : "Browser Preview"}
              currentModeBadge={overview ? backendModeToModeId[overview.mode] : "—"}
              currentModeDescription={
                overview
                  ? modeIdDescriptions[backendModeToModeId[overview.mode]]
                  : "浏览器预览模式下不可切换运行策略。"
              }
              modeOptions={(["hybrid", "strict-local"] as ModeId[]).map((modeId) => ({
                id: modeId,
              ...
              devAction={devAction}
              onDemoIngest={handleDemoIngest}
              onInitVault={handleInitVault}
              recentVaultPaths={recentVaultPaths}
              clearRecentVaultPaths={() => {
                setRecentVaultPaths([]);
              }}
              selectRecentVaultPath={setVaultPath}
              ingestUrlInput={ingestUrlInput}
              setIngestUrlInput={setIngestUrlInput}
              onUrlIngest={handleUrlIngest}
              queueEnqueueing={queueEnqueueing}
              enqueueUrl={(url) => {
                setQueueEnqueueing(true);
                return enqueueIngest("url", url)
                  .then(() => {
                    openOperationsModule("queue");
                  })
                  .catch((error: unknown) => {
              ...
              ingestFilePickedPaths={ingestFilePickedPaths}
              ingestFilePath={ingestFilePath}
              setIngestFilePath={setIngestFilePath}
              clearIngestFilePickedPaths={() => setIngestFilePickedPaths([])}
              pickIngestFiles={() =>
                pickFiles({
                  multiple: true,
                  filters: [
              ...
              defaultIngestFilePath={defaultIngestFilePath}
              ingestFileOcrProvider={ingestFileOcrProvider}
              ocrProviderLabels={ocrProviderLabels}
              setIngestFileOcrProvider={(provider) => {
                setIngestFileOcrProvider(provider);
                writeOcrProviderToStorage(provider);
                return saveOcrConfig(provider);
              }}
              onFileIngest={handleFileIngest}
              enqueueFiles={(paths) => {
                setQueueEnqueueing(true);
                return Promise.all(paths.map((path) => enqueueIngest("file", path)))
                  .then(() => {
                    openOperationsModule("queue");
                  })
                  .catch((error: unknown) => {
              ...
              clipServerOnline={clipServerOnline}
              clipServerPort={CLIP_SERVER_PORT}
              logs={logs}
            />
          )}
          {/* ---- Wiki 模块 ---- */}
          {activeModule === "wiki" && (
            <WikiModule
              isTauri={isTauriRuntime()}
              sortedWikiPages={sortedWikiPages}
              wikiSortMode={wikiSortMode}
              wikiSortModeLabels={wikiSortModeLabels}
              onWikiSortModeChange={setWikiSortMode}
              wikiKeyword={wikiKeyword}
              onWikiKeywordChange={setWikiKeyword}
              onWikiKeywordKeyDown={handleWikiKeywordKeyDown}
              wikiSearching={wikiSearching}
              onSearchWikiPages={handleSearchWikiPages}
              onResetWikiPages={handleResetWikiPages}
              showNewPageModal={showNewPageModal}
              onOpenNewPageModal={handleOpenNewWikiPageModal}
              onCloseNewPageModal={handleCloseNewWikiPageModal}
              newPageTopic={newPageTopic}
              onNewPageTopicChange={setNewPageTopic}
              newPageCreating={newPageCreating}
              onCreatePageWithAi={handleCreatePageWithAi}
              newPageResult={newPageResult}
              onUseNewPageResult={handleUseNewWikiPageResult}
              allWikiTags={allWikiTags}
              wikiActiveTags={wikiActiveTags}
              onClearWikiActiveTags={handleClearWikiActiveTags}
              onToggleWikiTag={handleToggleWikiActiveTag}
              displayedWikiPages={displayedWikiPages}
              onExpandAllWikiFolders={expandAllWikiFolders}
              onCollapseAllWikiFolders={collapseAllWikiFolders}
              renderWikiTree={renderWikiTree}
              wikiActivePagePath={wikiActivePagePath}
              wikiPageDetail={wikiPageDetail}
              isPageSummaryExpanded={isWikiPageSummaryExpanded}
              wikiSummaryPreviewLines={wikiSummaryPreviewLines}
              buildWikiSummaryDisplay={buildWikiSummaryDisplay}
              highlightWikiText={highlightWikiText}
              isPageActive={isWikiPageActive}
              isPageDetailActive={isWikiPageDetailActive}
              onToggleWikiSummary={handleToggleWikiSummary}
              wikiPageDetailLoading={wikiPageDetailLoading}
              onCloseWikiPreview={handleCloseWikiPreview}
              onOpenWikiPage={handleOpenWikiPage}
              wikiPageDetailError={wikiPageDetailError}
              renderWikiPreview={renderWikiPreview}
              isActiveWikiDetailInList={isActiveWikiDetailInList}
            />
          )}
          ...
          {activeModule === "ask" && (
            <AskModule
              isTauri={isTauriRuntime()}
              askSessionManaging={askSessionManaging}
              onCreateAskSession={handleCreateAskSession}
              askSessions={askSessions}
              askSessionKeyword={askSessionKeyword}
              setAskSessionKeyword={setAskSessionKeyword}
              askSessionSearchKeyword={askSessionSearchKeyword}
              setAskSessionSearchKeyword={setAskSessionSearchKeyword}
              askSessionSearching={askSessionSearching}
              onSearchAskSessionTurns={handleSearchAskSessionTurns}
              askSessionSearchHits={askSessionSearchHits}
              queryRunning={queryRunning}
              onOpenAskSearchHit={handleOpenAskSearchHit}
              formatAskSessionSearchSnippet={formatAskSessionSearchSnippet}
              formatAskHistoryCreatedAt={formatAskHistoryCreatedAt}
              askSessionsLoading={askSessionsLoading}
              filteredAskSessions={filteredAskSessions}
              askSessionId={askSessionId}
              onSelectAskSession={handleSelectAskSession}
              onRenameAskSession={handleRenameAskSession}
              onExportAskSession={handleExportAskSession}
              onDeleteAskSession={handleDeleteAskSession}
              askMessages={askMessages}
              queryHistoryItemsCount={queryHistoryItems.length}
              askHistoryKeyword={askHistoryKeyword}
              setAskHistoryKeyword={setAskHistoryKeyword}
              onClearQueryHistory={handleClearQueryHistory}
              filteredQueryHistoryItems={filteredQueryHistoryItems}
              setQueryQuestion={setQueryQuestion}
              askFocusedMessageId={askFocusedMessageId}
              expandedCitationIds={expandedCitationIds}
              setExpandedCitationIds={setExpandedCitationIds}
              askSearchDebugVisible={askSearchDebugVisible}
              formatQuerySearchStrategyLabel={formatQuerySearchStrategyLabel}
              formatQueryAnswerStrategyLabel={formatQueryAnswerStrategyLabel}
              formatQuerySearchRouteLabel={formatQuerySearchRouteLabel}
              searchDebugCopiedMessageId={searchDebugCopiedMessageId}
              onCopySearchDebug={handleCopySearchDebug}
              queryResult={queryResult}
              onSaveQueryResult={handleSaveQueryResult}
              queryResultSaving={queryResultSaving}
              messagesEndRef={messagesEndRef}
              showAskAdvanced={showAskAdvanced}
              setShowAskAdvanced={setShowAskAdvanced}
              queryTopKMin={queryTopKMin}
              queryTopKMax={queryTopKMax}
              queryTopK={queryTopK}
              setQueryTopK={setQueryTopK}
              setAskSearchDebugVisible={setAskSearchDebugVisible}
              onSaveQuerySettings={handleSaveQuerySettings}
              querySettingsSaving={querySettingsSaving}
              queryQuestion={queryQuestion}
              setQueryQuestionDirect={setQueryQuestion}
              onQueryAsk={handleQueryAsk}
          ...
          {activeModule === "graph" && (
            <GraphModule
              ForceGraph2D={ForceGraph2D as React.ComponentType<Record<string, unknown>>}
              GraphErrorBoundary={GraphErrorBoundary}
              graphInsightKindLabels={graphInsightKindLabels}
              graphViewMode={graphViewMode}
              graphSearchInputRef={graphSearchInputRef}
              graphSearchQuery={graphSearchQuery}
              setGraphSearchQuery={setGraphSearchQuery}
              graphGroupFilter={graphGroupFilter}
              setGraphGroupFilter={setGraphGroupFilter}
              graphGroupOptions={graphGroupOptions}
              graphLocalDepth={graphLocalDepth}
              graphSelectedNode={graphSelectedNode}
              graphLocalDirection={graphLocalDirection}
              graphShowOrphans={graphShowOrphans}
              setGraphShowOrphans={setGraphShowOrphans}
              graphNeighborOnly={graphNeighborOnly}
              setGraphNeighborOnly={setGraphNeighborOnly}
              graphVisibleData={graphVisibleData}
              graphLayoutFrozen={graphLayoutFrozen}
              graphRenderData={graphRenderData as any}
              graphAggregateMode={graphAggregateMode}
              GRAPH_AGGREGATE_THRESHOLD={GRAPH_AGGREGATE_THRESHOLD}
              graphLocalSubgraphTruncated={graphLocalSubgraphTruncated}
              graphNodes={graphNodes}
              graphNormalizedLinks={graphNormalizedLinks as any}
              graphVisibleOrphanCount={graphVisibleOrphanCount}
              graphMetrics={graphMetrics}
              graphShouldUseBackendSubgraph={graphShouldUseBackendSubgraph}
              graphLocalSubgraphLoading={graphLocalSubgraphLoading}
              graphLocalSubgraphError={graphLocalSubgraphError}
              ...
              isSameWikiPagePath={isSameWikiPagePath}
              graphSearchHits={graphSearchHits}
              resolveGraphNodePagePath={resolveGraphNodePagePath}
              setActiveModule={setActiveModule}
              handleOpenWikiPage={handleOpenWikiPage}
              handleGraphNodeClick={handleGraphNodeClick}
              graphSearchableNodes={graphSearchableNodes as any}
              graphSelectedAggregateId={graphSelectedAggregateId}
              setGraphAggregateMode={setGraphAggregateMode}
              handleGraphViewModeChange={handleGraphViewModeChange}
              GRAPH_LOCAL_DEPTH_MIN={GRAPH_LOCAL_DEPTH_MIN}
              GRAPH_LOCAL_DEPTH_MAX={GRAPH_LOCAL_DEPTH_MAX}
              handleGraphLocalDepthChange={handleGraphLocalDepthChange}
              handleGraphLocalDirectionChange={handleGraphLocalDirectionChange}
              handleGraphZoomToFit={handleGraphZoomToFit}
              handleToggleGraphLayoutFreeze={handleToggleGraphLayoutFreeze}
              handleExportGraphJson={handleExportGraphJson}
              handleResetGraphFilters={handleResetGraphFilters}
              handleGraphInsightSparseDensityChange={handleGraphInsightSparseDensityChange}
              graphInsightSparseDensity={graphInsightSparseDensity}
              GRAPH_INSIGHT_SPARSE_DENSITY_MIN={GRAPH_INSIGHT_SPARSE_DENSITY_MIN}
              GRAPH_INSIGHT_SPARSE_DENSITY_MAX={GRAPH_INSIGHT_SPARSE_DENSITY_MAX}
              handleGraphInsightBridgeMinGroupsChange={handleGraphInsightBridgeMinGroupsChange}
              graphInsightBridgeMinGroups={graphInsightBridgeMinGroups}
              GRAPH_INSIGHT_BRIDGE_MIN_GROUPS_MIN={GRAPH_INSIGHT_BRIDGE_MIN_GROUPS_MIN}
              GRAPH_INSIGHT_BRIDGE_MIN_GROUPS_MAX={GRAPH_INSIGHT_BRIDGE_MIN_GROUPS_MAX}
              handleGraphInsightSurprisingJaccardChange={handleGraphInsightSurprisingJaccardChange}
              graphInsightSurprisingJaccard={graphInsightSurprisingJaccard}
              GRAPH_INSIGHT_SURPRISING_JACCARD_MIN={GRAPH_INSIGHT_SURPRISING_JACCARD_MIN}
              GRAPH_INSIGHT_SURPRISING_JACCARD_MAX={GRAPH_INSIGHT_SURPRISING_JACCARD_MAX}
              handleGraphInsightSurprisingConfidenceChange={handleGraphInsightSurprisingConfidenceChange}
              graphInsightSurprisingConfidence={graphInsightSurprisingConfidence}
              GRAPH_INSIGHT_SURPRISING_CONFIDENCE_MIN={GRAPH_INSIGHT_SURPRISING_CONFIDENCE_MIN}
              GRAPH_INSIGHT_SURPRISING_CONFIDENCE_MAX={GRAPH_INSIGHT_SURPRISING_CONFIDENCE_MAX}
              graphInsights={graphInsights as any}
              handleApplyGraphInsight={handleApplyGraphInsight as any}
              graphSelectedAggregateNode={graphSelectedAggregateNode as any}
              graphSelectedAggregateMembers={graphSelectedAggregateMembers as any}
              handleExpandSelectedAggregateNode={handleExpandSelectedAggregateNode}
              handleExitAggregateMode={handleExitAggregateMode}
              handleOpenAggregateMemberPage={handleOpenAggregateMemberPage}
              handleOpenSelectedGraphNode={handleOpenSelectedGraphNode}
              graphSelectedNeighbors={graphSelectedNeighbors as any}
            />
          )}
